// Client bootstrap for interactive UI behavior on server-rendered EJS pages.
// Keep each initializer isolated so pages can opt in via selectors/data attributes.

// Enhances simple report forms by limiting subject options to selected content type.
function initReportTypeSubjectSelects() {
  document.querySelectorAll("form").forEach((form) => {
    ...
    // Hide unrelated subject options so backend always receives a valid pair.
...
// Enhanced subject picker for reports page (search text -> canonical subject ID).
function initEnhancedReportsSubjectSearch() {
  ...
  // Normalize text once so matching remains case-insensitive and whitespace-tolerant.
  ...
  // UI writes both visible label and hidden ID to keep backend payload unambiguous.
...
// Two-month date-range picker used only on reports page advanced generator.
function initReportsDateRangePicker() {
  ...
  // Parse YYYY-MM-DD safely; invalid values return null instead of broken Date.
  ...
  // Quarter buttons mirror selected range when range exactly matches full Q1/Q2/Q3/Q4.
  ...
  // Sync hidden form values (submitted to backend) and read-only display fields.
...
// Account image picker: converts selected file to data URL for prototype persistence.
...
// Topbar account dropdown with click-outside and Escape handling for accessibility.
...
// Safe to call unconditionally: each initializer exits early if required nodes are missing.
